...
class MyListViewController: UIViewController {
	...
	override func viewDidLoad() {
		...
		title = "\(AppSettings.selectedProfile?.userProfileName ?? "You") Watch List"
	...
	override func viewWillAppear(_ animated: Bool) {
		...
		viewModel.retrieveMyListFilms()
	}
	
	// MARK: - Private Methods
	
	private func bindViews(){
		viewModel.$myListFilms.sink { [weak self] _ in
...
extension MyListViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
	...
	func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
		...
		cell.configure(with: viewModel.myListFilms[indexPath.row])
		return cell
	}
	...
	func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
		return viewModel.myListFilms.count
	}
